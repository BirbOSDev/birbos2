...
OUTPUT = birb/boot/kernel.bin
BEAN = -vnc :0
run: all
	$(EMULATOR) $(EMULATOR_FLAGS) $(OUTPUT) $(BEAN)
...
build:all
	...
	rm birb/boot/grub/ -r -f
	mkdir birb/boot/grub/
	echo set default=0 >> birb/boot/grub/grub.cfg
	echo set timeout=0 >> birb/boot/grub/grub.cfg
	echo menuentry "birb" { >> birb/boot/grub/grub.cfg
	echo         set root='(hd96)' >> birb/boot/grub/grub.cfg
	echo         multiboot /boot/kernel.bin >> birb/boot/grub/grub.cfg
	echo } >> birb/boot/grub/grub.cfg

	grub-mkrescue -o birb.iso birb/
